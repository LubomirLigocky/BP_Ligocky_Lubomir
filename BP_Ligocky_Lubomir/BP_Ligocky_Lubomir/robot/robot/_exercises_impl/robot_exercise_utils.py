...
from face_follow import FaceTracker

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))

from robot.sadanie import stand_up_from_chair
...
class NaoqiConnection(object):
    ...
    def __init__(self, config):
        ...
        try:
            ...
            if self.lang.lower() == "en":
                self.speechProxy.setParameter("speed", 80)
            elif self.lang.lower() == "sk":
                self.speechProxy.setParameter("speed", 100)
            self.postureProxy = self.app.service("ALRobotPosture")
            self.motionProxy = self.app.service("ALMotion")
            #self.tracker_service = self.app.service("ALTracker")
        ...
        self.target_name = "Face"
        face_width = 0.1
        #self.tracker_service.registerTarget(self.target_name, face_width)
        #self.tracker_service.track(self.target_name)
        self.face_tracker = FaceTracker(self.ip_nao, self.port_nao, self.target_name, width=face_width)
        self.face_tracker.start_tracking()
        if self.check_if_robot_is_sitting():
            print("Not sitting")
            stand_up_from_chair_times = [[time for time in times] for times in
                                         stand_up_from_chair.times]
            self.motionProxy.angleInterpolationBezier(stand_up_from_chair.names, stand_up_from_chair_times,
                                                            stand_up_from_chair.keys)
        else:
            self.to_initial_pose()

    def check_if_robot_is_sitting(self):
        robot_position = self.motionProxy.getRobotPosition(False)
    ...
    def speak_or_message(self, sentence):
        ...
        #    self.face_tracker.restart_tracking()
...
class RobotExerciseUtils(object):
    ...
    hlasky_pool = {
        ...
        'forefooting_rozpazovanie': [
            '',
            'Zdvihňi pravú nohu a upaž ruky',
            'Polož nohu na zem a pripaž ruky',
            'Zdvihňi ľavú nohu a upaž ruky',
            'Polož nohu na zem a pripaž ruky',
        ],
        'forefooting_predpazovanie': [
            '',
            'Zdvihňi pravú nohu a predpaž ruky',
            'Polož nohu na zem a pripaž ruky',
            'Zdvihňi ľavú nohu a predpaž ruky',
            'Polož nohu na zem a pripaž ruky',
        ],
        'knee_lifting_rozpazovanie': [
            '',
    ...
    def __init__(self, naoqi_instance):
        ...
        self.naoqi.to_initial_pose()
        self.exercise_speach_lang = dict()
        self.base_speach_lang = dict()
        self.load_base_config(self.naoqi.lang)
        if self.naoqi.is_physical is False:
            self.FAST_MODE_MULTIPLIER = 1.5
        self.FAST_MODE_MULTIPLIER = 1
        self.naoqi.motionProxy.setFallManagerEnabled(True)

    def load_exercise_config(self, exercise_name, lang):
        path_to_exercise_file = "../lang/" + str(lang) + "/" + str(exercise_name) + ".json"
        print(path_to_exercise_file)
        with open(path_to_exercise_file, "r") as file:
            self.exercise_speach_lang = json.loads(file.read())

    def load_base_config(self, lang):
        path_to_base_file = "../lang/" + str(lang) + ".json"

        with open(path_to_base_file, "r") as file:
    ...
    def say_score(self, score, conn):
        if self.naoqi.er and score != self.naoqi.limit and (score == 2 or score == 4 ):
           ...
           #time.sleep(0.75)
        steps_speech_lang = self.base_speach_lang.get("template", {})
        if score == 0:
            time.sleep(0.01)
            #self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Pri cvičení nezabudňi plynule dýchať."))
        elif score == 1:
            self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Jeden."))
        elif score == 2:
            self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Dva."))
        elif score == 3:
            self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Tri, ako sa zatial cítiš?"))
        elif score == 4:
            self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Štyri."))
        elif score == 5:
            self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Päť."))
        elif score == 6:
            self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Šesť."))
        elif score == 7:
            self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Sedem, už len tri."))
        elif score == 8:
            self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Osem, už len dva."))
        elif score == 9:
            self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Ďeveť, ešťe jeden a máme to."))
        elif score == 10:
            self.naoqi.speak_or_message(steps_speech_lang.get(str(score), "Ďesať, super!"))
        else:
    ...
    def stop_tracker(self):
        try:
            self.naoqi.face_tracker.stop_tracking()
            #self.naoqi.tracker_service.stopTracker()
            #self.naoqi.tracker_service.unregisterAllTargets()
