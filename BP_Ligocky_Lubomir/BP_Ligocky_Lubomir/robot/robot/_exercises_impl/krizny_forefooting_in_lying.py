...
module_path = os.path.join(module_path, 'cross_forefooting')
if module_path not in sys.path:
    sys.path.append(module_path)
...
class KriznyforefootingInLying(RobotExerciseUtils):
    ...
    def __init__(self, naoqi_instance):
        ...
        self.exercise_name = "cross_forefooting_in_lying"
    ...
    def warning_say(self, message):
        if "reverse_pose" in message:
            sentence_to_say = self.warning_sentences.get("reverse_pose", "Zdvihli ste opačnú ruku a nohu.")
            ...
            sentence_to_say = self.warning_sentences.get("base_pose_1", "Prosím zdvihnite pravú nohu a ľavú ruku.")
            self.naoqi.speak_or_message(sentence_to_say)
            return

        elif "base_pose_2" in message:
            sentence_to_say = self.warning_sentences.get("base_pose_2", "Prosím zdvihnite ľavú nohu a pravú ruku.")
            self.naoqi.speak_or_message(sentence_to_say)
            return
        ...
        # *************************************
            ...
            sentence_to_say = self.warning_sentences.get("Iba_lava_ruka_nizko", "Ľavú ruku viacej vystriťe dohora.")
            self.naoqi.speak_or_message(sentence_to_say)
            ...
            sentence_to_say = self.warning_sentences.get("Iba_prava_ruka_nizko",
                                                         "Pravú ruku viacej vystriťe dohora.")
            self.naoqi.speak_or_message(sentence_to_say)
            ...
            sentence_to_say = self.warning_sentences.get("Ruka_a_noha_nizko",
                                                         "Vystretú ruku aj nohu otočťe viacej hore.")
            self.naoqi.speak_or_message(sentence_to_say)
        
        elif "Ruka_a_noha_daleko" in message:
            sentence_to_say = self.warning_sentences.get("Ruka_a_noha_daleko",
                                                         "Vystretá ruka a noha sú príliš posunuté k vašej hlave, dajťe ich viacej dohora.")
            self.naoqi.speak_or_message(sentence_to_say)
        ...
        # *************************************
            ...
            sentence_to_say = self.warning_sentences.get("Prava_noha_daleko_a_lava_ruka_nizko",
                                                         "Vystretú nohu aj ruku nemáťe v kolmej pozícií.")
            self.naoqi.speak_or_message(sentence_to_say)
