...
class ForefootingRozpazovanie(RobotExerciseUtils):
    ...
    def __init__(self, naoqi_instance):
        ...
        self.exercise_name = "forefooting_rozpazovanie"
        ...
        self.starting_sentence = self.exercise_speach_lang.get("on_start", {}).get(
            "default", "Začíname zdvíhať nohy na stoličke s upažovaňím. Sadňi si na stoličku.")
        ...
        self.warning_both_hands = self.warning_sentences.get("on_warning", {})
    ...
    def run_exercise(self, score, message, pending_messages, phase, conn):
        ...
        print(message)
        print(phase)
        print(self.finished_phases)
        if message == 'forefooting_rozpazovanie_start': #'forefooting_rozpazovanie_start,'
            if score == 0:
                self.reset()
        ...
        if message == 'forefooting_rozpazovanie_en':
            sentence_to_say = self.warning_sentences.get("on_phase", {}).get("forefooting_rozpazovanie_en",
                                                                             "Koniec cvičenia, Pripravíme sa na ďalší cvik.")
    ...
    def warning_say(self, message):
        if "In_Base_pos_left" in message:
            sentence_to_say = self.warning_sentences.get("In_Base_pos_left", "Rozpaž ruky a zdvihňi ľavú nohu.")
            self.naoqi.speak_or_message(sentence_to_say)
            return
        
        elif "In_Base_pos_right" in message:
            sentence_to_say = self.warning_sentences.get("In_Base_pos_right", "Rozpaž ruky a zdvihňi pravú nohu.")
            self.naoqi.speak_or_message(sentence_to_say)
            return
            ...
            if "ForefootingRoz_a_zdvihni_pravu_nohu" in message:
                sentence_to_say = self.warning_sentences.get("tpose_arms_above", {}).get(
                    "ForefootingRoz_a_zdvihni_pravu_nohu", "Posuň upažené ruky nižšie na vodorovnú polohu a zdvihňi pravú nohu.")
                self.naoqi.speak_or_message(sentence_to_say)
            elif "ForefootingRoz_a_zdvihni_lavu_nohu" in message:
                sentence_to_say = self.warning_sentences.get("tpose_arms_above", {}).get(
                    "ForefootingRoz_a_zdvihni_lavu_nohu", "Posuň upažené ruky nižšie na vodorovnú polohu a zdvihňi ľavú nohu.")
                self.naoqi.speak_or_message(sentence_to_say)
            else:
                sentence_to_say = self.warning_sentences.get("tpose_arms_above", {}).get(
            ...
            if "ForefootingRoz_a_zdvihni_pravu_nohu" in message:
                sentence_to_say = self.warning_sentences.get("tpose_arms_below", {}).get(
                    "ForefootingRoz_a_zdvihni_pravu_nohu",
                    "Posuň upažené ruky vyššie na vodorovnú polohu a zdvihňi pravu nohu.")
                self.naoqi.speak_or_message(sentence_to_say)
                
            elif "ForefootingRoz_a_zdvihni_lavu_nohu" in message:
                sentence_to_say = self.warning_sentences.get("tpose_arms_below", {}).get(
                    "ForefootingRoz_a_zdvihni_lavu_nohu",
                    "Posuň upažené ruky vyššie na vodorovnú polohu a zdvihňi ľavú nohu.")
                self.naoqi.speak_or_message(sentence_to_say)
            else:
                sentence_to_say = self.warning_sentences.get("tpose_arms_below", {}).get(
            ...
            if "ForefootingRoz_a_zdvihni_pravu_nohu" in message:
                sentence_to_say = self.warning_sentences.get("tpose_arms_wrong", {}).get(
                    "ForefootingRoz_a_zdvihni_pravu_nohu",
                    "Skús rozpažiť vodorovne a zdvihňi pravú nohu.")
                self.naoqi.speak_or_message(sentence_to_say)
                
            elif "ForefootingRoz_a_zdvihni_lavu_nohu" in message:
                sentence_to_say = self.warning_sentences.get("tpose_arms_wrong", {}).get(
                    "ForefootingRoz_a_zdvihni_lavu_nohu",
                    "Skús rozpažiť vodorovne a zdvihňi ľavú nohu.")
                self.naoqi.speak_or_message(sentence_to_say)
            else:
                sentence_to_say = self.warning_sentences.get("tpose_arms_wrong", {}).get(
            ...
            sentence_to_say = self.warning_sentences.get("ForefootingRoz_a_zdvihni_pravu_nohu", "Zdvihňi pravú nohu.")
            self.naoqi.speak_or_message(sentence_to_say)
            
        elif "ForefootingRoz_a_zdvihni_lavu_nohu" in message:
            sentence_to_say = self.warning_sentences.get("ForefootingRoz_a_zdvihni_pravu_nohu", "Zdvihňi ľavú nohu.")
            self.naoqi.speak_or_message(sentence_to_say)
