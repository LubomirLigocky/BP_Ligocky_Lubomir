...
module_path = os.path.join(os.getcwd(), '_exercises_impl')
if module_path not in sys.path:
    sys.path.append(module_path)

from forefooting_in_lying import ForefootingInLying
from krizny_forefooting_in_lying import KriznyforefootingInLying
from robot_exercise_utils import NaoqiConnection
from predpazovanie import Predpazovanie
from forefooting_rozpazovanie import ForefootingRozpazovanie
from forefooting_predpazovanie import ForefootingPredpazovanie
from knee_lifting_rozpazovanie import KneeLiftingRozpazovanie
from knee_lifting_predpazovanie import KneeLiftingPredpazovanie
from drepy import Drepy
from upazovanie import Upazovanie
from kruzenie_v_stoji import KruzenieVStoji
from kruzenie_v_sede import KruzenieVSede
from obchadzanie_stolicky import ObchadzanieOkoloStolicky
...
def voice_server_loop():
    """Beží na pozadí a čaká na text z OpenAI modelu z PC."""
    HOST = "0.0.0.0" 
    PORT = 5005
    ROBOT_IP = "10.62.44.100"
    ROBOT_PORT = 9559

    tts = ALProxy("ALTextToSpeech", ROBOT_IP, ROBOT_PORT)
    tts.setParameter("speed", 75)   # slower speaking for elderly people
    ...
    server.listen(1)
    print("NAO Hlasový server beží na porte 5005")
    ...
    while True:
        try:
            ...
            if data:
                ...
                print("\n[HLAS] NAO Hovorí:", text)
                
                try:
                    tts.say(text.encode('utf-8'))                    
                    conn.sendall("FINISHED_SPEAKING".encode('utf-8'))                    
                    print("NAO dohovoril: " + text.encode('ascii', 'ignore'))
                except Exception as e:
                    print("Chyba potvrdenia:", e)
                    
            conn.close()
        except Exception as e:
            print("Chyba vo voice serveri:", e)
...
class RobotTrainer(object):
    ...
    def __init__(self):
        ...
        print("Cvičebný server beží na porte:", self.PORT)
    ...
    def robot_loop(self):
        message = "init"
        print("Waiting for exercise commands.....")
        ...
        while message:
            ...
            print('\n[CVIČENIE] Received from camera app: ', message)
            ...
            if "config" in message:
                ...
                if len(spl) == 2:
                    lang = spl[1].lower()
                    if self.naoqi_instance:
                        self.naoqi_instance.lang = lang
    ...
    def exercise_in_queue(self, message, pending_messages = []):
        if "fullfilled" not in message:
            ...
            elif "knee_lifting_rozpazovanie_end," in message:
                message = message.replace("knee_lifting_rozpazovanie_end,", "knee_lifting_rozpazovanie_en")

            elif "knee_lifting_predpazovanie_end," in message:
                message = message.replace("knee_lifting_predpazovanie_end,", "knee_lifting_predpazovanie_en")
        ...
        while pending_messages and (self.use_queue is True):
            ...
            if message == 'forefooting_in_lying_start,':
                self.curr_exercise = ForefootingInLying(self.naoqi_instance)
            ...
            if message == 'knee_lifting_rozpazovanie_start,':
                self.curr_exercise = KneeLiftingRozpazovanie(self.naoqi_instance)

            if message == 'knee_lifting_predpazovanie_start,':
                self.curr_exercise = KneeLiftingPredpazovanie(self.naoqi_instance)
    ...
    def exercise_not_in_queue(self, message, pending_messages = []):
        ...
        elif message == 'forefooting_rozpazovanie_start':
            self.curr_exercise = ForefootingRozpazovanie(self.naoqi_instance)
        elif message == 'forefooting_predpazovanie_start':
            self.curr_exercise = ForefootingPredpazovanie(self.naoqi_instance)
        elif message == 'knee_lifting_rozpazovanie_start':
            self.curr_exercise = KneeLiftingRozpazovanie(self.naoqi_instance)
        elif message == 'knee_lifting_predpazovanie_start':
            self.curr_exercise = KneeLiftingPredpazovanie(self.naoqi_instance)
...
if __name__ == "__main__":
    # voice thread server in the background
    voice_thread = threading.Thread(target=voice_server_loop)
    voice_thread.daemon = True # turning off with main program
    voice_thread.start()
