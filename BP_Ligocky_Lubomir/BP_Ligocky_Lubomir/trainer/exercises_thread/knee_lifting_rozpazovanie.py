from exercise_poses.knee_lifting_rozpazovanie import KneeLiftingRozpazovaniePose
from .exercise import Exercise
import configuration.exercises as exercise_messages_configuration

class KneeLiftingRozpazovanie(Exercise):
    def __init__(self, messages, widget, app_instance):
        super(KneeLiftingRozpazovanie, self).__init__(messages, widget, app_instance)
        self.start_msg = messages['start_msg'] + ","

    def knee_lifting_rozpazovanie_exercise(self):
        self.start_timer()

        self.camera_thread.current_exercise = KneeLiftingRozpazovaniePose()
