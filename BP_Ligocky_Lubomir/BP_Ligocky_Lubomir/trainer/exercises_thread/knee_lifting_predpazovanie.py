from exercise_poses.knee_lifting_predpazovanie import KneeLiftingPredpazovaniePose
from .exercise import Exercise
import configuration.exercises as exercise_messages_configuration

class KneeLiftingPredpazovanie(Exercise):
    def __init__(self, messages, widget, app_instance):
        super(KneeLiftingPredpazovanie, self).__init__(messages, widget, app_instance)
        self.start_msg = messages['start_msg'] + ","

    def knee_lifting_predpazovanie_exercise(self):
        self.start_timer()

        self.camera_thread.current_exercise = KneeLiftingPredpazovaniePose()
