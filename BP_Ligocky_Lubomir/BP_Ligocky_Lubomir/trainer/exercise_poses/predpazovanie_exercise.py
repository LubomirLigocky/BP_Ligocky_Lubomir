...
class PredpazovanieExercisePose(ExerciseBase):
    ...
    def do_check_exercise(self, landmarks, stage_signal, exercise_label_signal, score_signal, depth = None, frame = None):
        # print(self.arms_poses.is_arms_raised_forward(landmarks))

        if self.side_camera and not self.side_camera.disabled:
            is_arms_raised_forward, arms = self.arms_poses.is_arms_raised_forward(landmarks, self.width_threshold, side_camera=self.side_camera)
        else:
            is_arms_raised_forward, arms = self.arms_poses.is_arms_raised_forward(landmarks, self.width_threshold, depth = depth, frame = frame)
        ...
        if self.predpazovanie_sent01 == False and self.predpazovanie_sent00 == True:
            if self.side_camera and not self.side_camera.disabled:
                for side, status in arms.items():
                     if status and status["coordinates"] is not None:
                         if (front_or_back := status["coordinates"]["front_or_back"]) and front_or_back == "back":
                             print("Predpazovanie_nie_je_zapazovanie ee:" + front_or_back)
                             self.message_sent = "Predpazovanie_nie_je_zapazovanie"
            ...
            if is_arms_raised_forward is True:
                ...
                if time.time() - self.correct_pose_time_treshold >= 0.5:
                ...
                waiting_threshold = 3.0
                if self.message_sent != emit_msg:
                    waiting_threshold = 1.5
                print("Waiting" + str(waiting_threshold))
                if self.check_wrong_pose_timer(waiting_threshold, False):
        ...
        elif self.predpazovanie_sent01 == True and self.predpazovanie_sent00 == True:
            ...
            elif self.check_wrong_pose_timer(1.5, False):
    ...
    def warning_message(self, both_arms, side_arm, right_arm_up, left_arm_up, right_arm_low,
                        left_arm_low, right_arm_aside, left_arm_aside, front_or_back: str="front"):
        print("WARNING: " + front_or_back)
        if front_or_back and front_or_back == "back":
            print("Predpazovanie_nie_je_zapazovanie: " + front_or_back)
            return "Predpazovanie_nie_je_zapazovanie"
        if both_arms:
