...
class TposeExercisePose(ExerciseBase):
    ...
    def do_check_exercise(self, landmarks, stage_signal, exercise_label_signal, score_signal, depth = None, frame = None):
        ...
        is_in_tpose, arms_coordinates, is_wrist_close = self.arms_poses.is_arms_in_tpose(landmarks, self.y_threshold,
                                                                                         self.wrist_shoulder_distance_threshold)
        ...
        if self.stage_up is False and self.stage_down is False:
            ...
            elif self.check_wrong_pose_timer(2.0, False):
            ...
            if is_in_tpose:
                ...
                if is_arms_raised_forward:
                    self.wrong_pose_time_treshold = None
                    self.correct_pose_time_treshold = None

                    if self.check_base_pose_timer(2.5):
                        self.message_sent = 'Foreground_hands_pos'
                        stage_signal.emit("warning", self.message_sent, self.camera_exerice_score)
                        self.exercise_lock = True
                        exercise_label_signal.emit(self.message_sent)
        ...
        if both_arms:
            if right_arm_up and left_arm_up:
                if right_arm_aside or left_arm_aside:
                    return "V_tPoze_Upazene_ruky_vysoko_a_odseba"
                else:
                    return "V_tPoze_Upazene_ruky_vysoko"
            elif right_arm_low and left_arm_low:
                if right_arm_aside or left_arm_aside:
                    return "V_tPoze_Upazene_ruky_nizko_a_odseba"
                else:
                    return "V_tPoze_Upazene_ruky_nizko"
            else:
                if (right_arm_up and left_arm_low) or (left_arm_up and right_arm_low):
                    return  "V_tPoze_Upazene_ruky_zle"
                elif right_arm_aside or left_arm_aside:
                    return  "V_tPoze_Upazene_ruky_priliz_od_seba"
        elif side_arm == "right":
            if right_arm_up:
                return "V_tPoze_prava_ruka_vysoko"
            elif right_arm_low:
                return "V_tPoze_prava_ruka_nizko"
            else:
                return "V_tPoze_prava_ruka_nespravne"
        elif side_arm == "left":
            if left_arm_up:
                return "V_tPoze_lava_ruka_vysoko"
            elif left_arm_low:
                return "V_tPoze_lava_ruka_nizko"
            else:
                return "V_tPoze_lava_ruka_nespravne"
        return "Chyba"
