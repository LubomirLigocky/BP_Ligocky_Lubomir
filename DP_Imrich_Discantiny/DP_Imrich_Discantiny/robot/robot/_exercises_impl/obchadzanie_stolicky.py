...
class ObchadzanieOkoloStolicky(RobotExerciseUtils):
    ...
    def perform_chair_circling(self, repetitions=5, step_distance=1.0):
        ...
        for i in range(repetitions):
            ...
            for _ in range(4):  
                ...
                dx = step_distance * math.cos(self.orientation)
                dy = step_distance * math.sin(self.orientation)
                self.position[0] += dx
                self.position[1] += dy
                print(f"Moved to position: {self.position}, orientation: {math.degrees(self.orientation):.1f}°")

               
                self.orientation += self.rotation_angle
                self.orientation = self.orientation % (2 * math.pi)
                print(f"Rotated to new orientation: {math.degrees(self.orientation):.1f}°")
